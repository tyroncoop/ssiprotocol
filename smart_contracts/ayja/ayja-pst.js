export function handle(state, action) {
    ...
    if (input.function === 'transfer') {
        ...
        if (!beneficiary || typeof beneficiary !== 'string') {
            throw new ContractError(
                `Beneficiary is not specified or it has a wrong format.`
            )
        }
        ...
        if (accounts[username].balance < amount) {
            throw new ContractError(
                `Originator's balance not high enough to make this transfer.`
            )
        }
    ...
    if (input.function === 'updateWallet') {
        ...
        if (!wallet || typeof wallet !== 'string') {
            throw new ContractError(
                `Originator's permawallet address not given or has a wrong format: ${wallet}.`
            )
        }
